...
//Proje sunum videosu linki: https://www.youtube.com/watch?v=Fxsl2OedsUA
...
void saveGame(char** game_board, int size, PLAYER* player1, PLAYER* player2);
	...
	do{
		...
		switch(menu){
			...
			case 1: //create a new game
				...
				system("CLS");
				break;
			case 2: //load a game from a txt file
				printf("1 vs 1: 1 | 1 vs Computer: 2\nYour choice: ");
				scanf("%d", &gametype);
...
char** createMatrix(int size){ // allocates matrix
	char** matrix;
	int i,j;
...
char** createGameBoard(int size){
	...
	int i,j;
	srand(time(NULL));
	//fills game board randomly with A,B,C,D,E
	for(i=0;i<size;i++){
		for(j=0;j<size;j++){
			matrix[i][j] = 'A' + (rand() % 5);
		}
	}
	//puts 4 spaces in the center of the game board
...
void printGame(char** matrix, int size, PLAYER* player1, PLAYER* player2, MOVE_DATA* move_data){
    //prints the gameboard, player's score and stone count.
	int i,j;
...
int isGameOver(char** game_board, int size){
	//controls that there are an avaible move on the game board.
	int i=0,j=0,flag=0;	
	while(i<size && flag!=1){
		while(j<size && flag!=1){			
...
int isSelectValid(char** game_board, int size, MOVE_DATA* move_data){ //checks the accuracy of the stone that selected by the user.
	int i,j;
...
int isMoveValid(char** game_board, int size, MOVE_DATA* move_data){ //checks the accuracy of the move location
...
void saveGame(char** game_board, int size, PLAYER* player1, PLAYER* player2){ //saves the game in a txt file
    
    char filename[20];
	printf("\nGive a name that game to save: ");
	scanf("%s", filename);
	strcat(filename, ".txt");
...
void loadGame(char*** game_board, int* size, PLAYER* player1, PLAYER* player2){ //loads the game saved in the txt file
    ...
    FILE* file;
    do{
		printf("\n\nWrite the name of the game that you want to play: ");
		scanf("%s", &filename);
		strcat(filename, ".txt");
...
int findScore(PLAYER* player){ //calculates player's score
	int i;
...
void savePrevGame(GAME_DATA* prev_game, PLAYER* player, MOVE_DATA* move_data, char** game_board, int size){ //saves the game before a move is made
	int i,j;
...
int computerSelectValid(char** game_board, int size, MOVE_DATA* move_data, char wanted1, char wanted2){ 
	...
	//controls the correctness of the stone selected by the computer
	int i,j;
	i= move_data->selected_stone_x;
	j= move_data->selected_stone_y;
...
void twoPlayerGame(int neworcont){
	...
	//creates a new game or installs a previous game
	if(neworcont == 0){
		system("CLS");
	...
	do{
		...
		while(flag != 0 && control != 0){
			...
			savePrevGame(prev_game, player2, move_data, game_board, size);
			makeMove(game_board, size, player2, move_data);
			flag = isSelectValid(game_board, size, move_data);
			if(undo!=0){
				system("CLS");
				printf("Player %s's turn\n----------------------\n\n\n", player2->name);
				printGame(game_board, size, player1, player2, move_data);
				askUndo(&undo, prev_game, player2, player1, player2, move_data, game_board, size, &flag);
			}
		...
		if(control!=0){
			...
			scanf("%d", &savenquit);
			if(savenquit == 0){
				printf("\n[i]-> Save the game and quit: 1 | Save the game without quitting: 2 | Quit the game without saving: 3\nYour choice: ");
				scanf("%d", &savenquit);
				if(savenquit != 3){
					saveGame(game_board, size, player1, player2);
...
void computerGame(int neworcont){
	...
	if(neworcont == 0){
		...
		computer->score = 0;
		for(i=0;i<5;i++){
			computer->colors_count[i] = 0;
		}		
	}
	else{
		loadGame(&game_board, &size, player1, computer);
	}
	...
	do{
		...
		while(flag != 0 && control != 0){
			system("CLS");
			printf("Player %s's turn\n----------------------\n\n\n", player1->name);
			printGame(game_board, size, player1, computer, move_data);
			if(move_data->isFirstMove == 1){
				selectStone(game_board, size, move_data);
				system("CLS");
				printf("Player %s's turn\n----------------------\n\n\n", player1->name);
			...
			if(undo!=0){
				...
				printf("Player %s's turn\n----------------------\n\n\n", player1->name);
				printGame(game_board, size, player1, computer, move_data);
				askUndo(&undo, prev_game, player1, player1, computer, move_data, game_board, size, &flag);
			}
		}
		...
		//Computer's turn
		if(control!=0){
			computerMakeMove(game_board, size, player1, computer, move_data);
			system("CLS");
			printf("Player %s's turn\n----------------------\n\n\n", computer->name);
			printGame(game_board, size, player1, computer, move_data);
			printf("\n[?]-> Would you like to quit that game?\nYes: 0 | No: 1\nYour choice: ");
			...
			if(savenquit == 0){
				...
				scanf("%d", &savenquit);
				if(savenquit != 3){
					saveGame(game_board, size, player1, computer);
					if(savenquit == 1) control = -1;	
				}
				else control = 0;
			}	
